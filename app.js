(function () {
  ...
  function showLanding() { document.body.classList.remove("chat-active"); panel.hidden = true; if (authenticated) { form.hidden = true; hideRequestPanels(); requestPanel.hidden = false; return; } form.hidden = false; fields.hidden = pinMode !== "cedula"; pinFields.hidden = pinMode === "cedula"; pinConfirmBox.hidden = pinMode !== "enroll"; hideRequestPanels(); button.disabled = false; button.textContent = pinMode === "enroll" ? "Crear PIN y continuar" : pinMode === "verify" ? "Ingresar a GAIA" : "Continuar con GAIA"; }
