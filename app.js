(function () {
  ...
  let updateKioskScroll = function () {};
  ...
  function showKeypad(target) { activateKeypad(target); if (!touchKeypad) return; touchKeypad.hidden = false; document.body.classList.add("keypad-open"); }
  function hideKeypad() { if (!touchKeypad) return; touchKeypad.hidden = true; document.body.classList.remove("keypad-open"); }
  function showLanding() { document.body.classList.remove("chat-active"); panel.hidden = true; if (authenticated) { form.hidden = true; hideRequestPanels(); requestPanel.hidden = false; requestAnimationFrame(updateKioskScroll); return; } form.hidden = false; fields.hidden = pinMode !== "cedula"; pinFields.hidden = pinMode === "cedula"; pinConfirmBox.hidden = pinMode !== "enroll"; hideRequestPanels(); button.disabled = false; button.textContent = pinMode === "enroll" ? "Crear PIN y continuar" : pinMode === "verify" ? "Ingresar a GAIA" : "Continuar con GAIA"; activateKeypad(pinMode === "cedula" ? input : pinInput); }
  ...
  if (touchKeypad) document.addEventListener("pointerdown", function (event) { if (!event.target.closest("#touch-keypad, #cedula, #pin, #confirmar-pin")) hideKeypad(); });
  if (touchKeypad) touchKeypad.querySelectorAll("[data-keypad-key]").forEach(function (key) { key.addEventListener("click", function () { const value = key.dataset.keypadKey; if (value === "enter") { hideKeypad(); form.requestSubmit(); return; } if (!keypadTarget) return; if (value === "clear") keypadTarget.value = ""; else if (value === "backspace") keypadTarget.value = keypadTarget.value.slice(0, -1); else if (keypadTarget.value.length < Number(keypadTarget.maxLength || 99)) keypadTarget.value += value; keypadTarget.dispatchEvent(new Event("input", { bubbles: true })); keypadTarget.focus({ preventScroll: true }); }); });
  const kioskScroll = document.getElementById("kiosk-scroll-next");
  if (kioskScroll) { updateKioskScroll = function () { const bottom = window.scrollY + window.innerHeight >= document.documentElement.scrollHeight - 16; kioskScroll.textContent = bottom ? "↑" : "↓"; kioskScroll.setAttribute("aria-label", bottom ? "Volver arriba" : "Ver más servicios"); }; kioskScroll.addEventListener("click", function () { const bottom = window.scrollY + window.innerHeight >= document.documentElement.scrollHeight - 16; window.scrollTo({ top: bottom ? 0 : window.scrollY + Math.max(280, Math.round(window.innerHeight * .62)), behavior: "smooth" }); }); window.addEventListener("scroll", updateKioskScroll, { passive: true }); window.addEventListener("resize", updateKioskScroll); updateKioskScroll(); }
  ...
  document.getElementById("volver-solicitudes").addEventListener("click", showLanding); document.getElementById("cambiar-solicitud").addEventListener("click", function () { showFlow(selectedFlow); }); document.getElementById("abrir-solicitud").addEventListener("click", startNewConversation); document.getElementById("mis-conversaciones").addEventListener("click", openExistingConversations); document.getElementById("volver").addEventListener("click", showLanding); document.getElementById("cerrar-sesion").addEventListener("click", function () { endSession("Tu sesión fue cerrada."); }); const catalogLogout = document.getElementById("cerrar-sesion-catalogo"); if (catalogLogout) catalogLogout.addEventListener("click", function () { endSession("Tu sesión fue cerrada."); }); window.addEventListener("pagehide", function () { clearInactivityTimer(); input.value = ""; });
