(function () {
  ...
  let widgetAvailable = false, authenticated = false, currentCedula = "", selectedFlow = "", attempt = 0, timeout;
  ...
  function showFlow(flow) {
    selectedFlow = flow; requestPanel.hidden = confirmPanel.hidden = true; detailTitle.textContent = flows[flow].title; detailChoices.replaceChildren();
    flows[flow].choices.forEach(function (choice) { const option = document.createElement("button"); option.type = "button"; option.className = "detail-choice"; option.textContent = choice; option.addEventListener("click", function () { detailPanel.hidden = true; selectionSummary.textContent = `${flowLabels[selectedFlow]} · ${choice}`; confirmPanel.hidden = false; }); detailChoices.appendChild(option); });
  ...
  document.getElementById("volver-solicitudes").addEventListener("click", showLanding); document.getElementById("cambiar-solicitud").addEventListener("click", function () { showFlow(selectedFlow); }); document.getElementById("abrir-solicitud").addEventListener("click", openConversation); document.getElementById("mis-conversaciones").addEventListener("click", openConversation); document.getElementById("volver").addEventListener("click", showLanding); window.addEventListener("pagehide", function () { input.value = ""; });
